 ...
 * @param  {Object} options                pool max length, worker timeout
 * @param  {Object||Integer} cacheOptions  capacity and expires
 * @return {Object}                        generated pool
 */
exports.generatePool = function (worker, options, cacheOptions) {
  var count = 0;
  options = options || {};
  if (typeof options !== 'object') {
    throw new Error('generate pool error, options must be an object');
  }
  var cache = new LRU(cacheOptions);
  var concurrency = options.concurrency || 1;
  var timeoutLength = options.timeout || 2 * 60 * 1000;
  ...
  function wrapWorker(task, callback) {
    var timeout;
    var taskTimeout = setTimeout(function () {
      timeout = true;
      callback(new Error('task works timeout'));
    }, timeoutLength);
  ...
  var pool = {
    ...
    process: function () {
      ...
      if (pool.waitings && pool.waitings.length) {
        ...
        if (!workingQueue) {
          workingQueue = async.queue(wrapWorker, 1);
        }
        count += 1;
        task = pool.waitings.shift();
